...
<p align="center">
  <img src="./docs/images/start_menu.png" width="800" alt="Главный экран Lab Parser">
</p>
...
- Позволяет собрать, распарсить, проверить и оформить согласно формату десятки отчетов в единую Excel-таблицу.
...
- Собирает в отдельный лог ненайденные при парсинге ЭВСД 

## 2. Требования
- ОС: Windows 10/11 (x64)*
- Excel: Microsoft Excel 2007+ (обязательно!)
- Программа работает "из коробки" — дополнительные библиотеки не нужны (.exe самодостаточен)
...
## 3. Как пользоваться
1) Подготовить номера ТТН, т.е. создать 4 разных .xlsx или .txt под каждый сегмент, записав в них построчно каждый номер ТТН.
Примеры:
фоточек накинуть

2) Выбрать файл из п.1: "Обзор" - указать полный путь до конкретного списка ТТН, 
отметить нужный ХС (ТД / ЧМПЗ), указать с какой даты оформления выгружать и нажать "Загрузить ЭВСД".
<p align="center">
  <img src="./docs/images/init.png" width="800" alt="Инициализация выгрузки">
</p>

3) Начнет отрабатывать блок автоматизации выгрузки из Меркурия. На моменте с QR-кодом нам необходимо его отсканировать не более чем за 60 с.
P.S. потребуется установленное приложение Госуслуг.
<p align="center">
  <img src="./docs/images/qr_screen.png" width="800" alt="Экран с QR-кодом">
</p>

4) Ожидаем окончания загрузки всех печатных форм ЭВСД, триггер - закрытие браузера и сообщение:
- а) Успех, все ТТН были найдены
пикча без лога
- б) Частично успех, сформирован лог с ненайденными ТТН
пикча с логом

P.S.: примерная скорость обработки - 5 ТТН в минуту.

5) Нам открывается дальнейший функционал парсера - разбивка всех выгруженных .xlsx в единую таблицу с эталонным форматом.
Прожимаем кнопку "Обработать":
пикчу с обработать
...
- **v2.0** - реализован парсинг ЭВСД из ГИС "Меркурий"
